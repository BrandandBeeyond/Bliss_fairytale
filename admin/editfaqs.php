...
if (!isset($_SESSION['email'])) {
    ...
    if (isset($_GET['editfaqs'])) {
        ...
        $faqId = $_GET['editfaqs'];
        $getFAQSql = "SELECT * FROM faqs WHERE id='$faqId'";
        $getFAQ = mysqli_query($conn, $getFAQSql);
        $getFAQRow = mysqli_fetch_array($getFAQ);

        // Fetch data from the database
        $question = $getFAQRow['question'];
        $answer = $getFAQRow['answer'];
        $aos_animation = $getFAQRow['aos_animation'];
        $aos_offset = $getFAQRow['aos_offset'];
        ?>
                                ...
                                <div class="card-body">
                                    <form class="needs-validation" novalidate method="post" enctype="multipart/form-data">
                                        <div class="row">
                                            ...
                                            <div class="col-md-12">
                                                <div class="mb-3">
                                                    <label class="form-label" for="specialityname">question</label>
                                                    <input type="text" class="form-control" id="specialityname"
                                                        placeholder="Speciality Name" name="question"
                                                        value="<?php echo htmlspecialchars($question); ?>" required>
                                                </div>
                                            </div>
                                            <div class="col-md-12">
                                                <div class="mb-3">
                                                    <label class="form-label" for="patientfeedback">answer</label>
                                                    <textarea name="answer" id="answer" class="form-control ckeditor"
                                        ...
                                        <!-- Submit Button -->
                                        <button type="submit" name="updatefaqs" class="btn btn-primary">
