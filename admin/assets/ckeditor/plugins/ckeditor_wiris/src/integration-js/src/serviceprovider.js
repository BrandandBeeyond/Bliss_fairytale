...
export default class ServiceProvider {
  ...
  static getUrl(url, postVariables) {
    ...
    const Request = Util.createHttpRequest();

    if (Request) {
      if (typeof postVariables === 'undefined' || typeof postVariables === 'undefined') {
        Request.open('GET', url, false);
      } else if (url.substr(0, 1) === '/' || url.substr(0, 7) === '://' || url.substr(0, 8) === 's://') {
        Request.open('POST', url, false);
      } else {
        Request.open('POST', currentPath + url, false);
      }

      if (typeof postVariables !== 'undefined' && postVariables) {
        Request.setRequestHeader('Content-type', 'application/x-www-form-urlencoded; charset=UTF-8');
        Request.send(Util.BuildQuery(postVariables));
      } else {
        Request.send(null);
      }
