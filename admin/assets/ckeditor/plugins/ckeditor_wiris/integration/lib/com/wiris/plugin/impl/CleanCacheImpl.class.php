...
class com_wiris_plugin_impl_CleanCacheImpl implements com_wiris_plugin_api_CleanCache{
	...
	public function getCacheOutput() {
		if($this->gui) {
			...
			$output .= "<title>MathType integration clean cache service</title><meta -equiv=\"content-type\" content=\"text/html; charset=UTF-8\" />\x0D\x0A";
			...
			if($this->token !== null && !$this->validToken) {
				...
				if($this->validToken && $this->token !== null) {
					...
					if($this->newToken !== null) {
						...
						$output .= " For more information see <a href=://www.wiris.com/plugins/docs/resources/configuration-table style=text-decoration:none>Server configuration file documentation</a>";
					}
				}
			}
			$output .= "</div>";
